...
double SimulationHedger::hedging_PL(MonteCarloPricer *monteCarloPricer, int H, bool isParallel)
{
    ...
    PnlVect *spot;
    double priceProduct = monteCarloPricer->prod->payoff(pathPayoff,spot);
...
void SimulationHedger::prices(MonteCarloPricer *monteCarloPricer, int H, char* fileProductName, char* fileTimeName, bool isParallel) {
    ...
    if (fileProduct && fileTime) {
        ...
        std::cout << "\nPour visualiser les cours des actifs et celui du produit, lancer les scripts R " << " \n";
    }
